...
class COCODataset:
    ...
    def generate_coco_filename(self):
    ...
    def find_next_images_path(self):
        while self.image_id < self.max_image_id:
            potential_path = pathlib.PurePath(self.coco_directory, self.generate_coco_filename())
    ...
    def get_path_to_image(self):
        path = self.find_next_images_path()
        if not path:
            if self.run_in_loop:
                self.image_id = 0
                path = self.find_next_images_path()
    ...
    def resize_and_crop_image(self, image_array):
    ...
    def get_next_image(self):
        image_array = cv2.imread(self.get_path_to_image())
        if self.allow_distortion:
            image_array = cv2.resize(image_array, self.shape)
        else:
            image_array = self.resize_and_crop_image(image_array)
        self.image_id += 1
        return np.expand_dims(image_array, axis=0).astype(self.input_data_type)

    def get_input_array(self):
        input_array = self.get_next_image()
        for _ in range(1, self.batch_size):
            input_array = np.concatenate((input_array, self.get_next_image()), axis=0)
