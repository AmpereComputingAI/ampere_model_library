...
def get_env_variable(env_var_name: str, fail_message: str):
    try:
        ...
        utils.print_goodbye_message_and_die(fail_message)


class COCODataset(ImageDataset):
    def __init__(self,
                 batch_size,
                 images_path=None, annotations_path=None, allow_distortion=True, pre_processing_func=None):

        if images_path is None:
            env_var = "COCO_IMG_PATH"
            images_path = get_env_variable(
                env_var, f"Path to COCO images directory has not been specified with {env_var} flag")
        if annotations_path is None:
            env_var = "COCO_ANNO_PATH"
            annotations_path = get_env_variable(
    ...
    class OutOfCOCOImages(Exception):
        def __init__(self, message="No more COCO images to process in the directory provided"):
            super(OutOfCOCOImages, self).__init__(message)

    def __get_path_to_img(self):
        try:
            image_id = self.__image_ids[self.__current_img]
        except KeyError:
            raise OutOfCOCOImages("No more COCO images to process")
    ...
    def get_input_array(self, target_shape):
        ...
        for _ in range(1, self.__batch_size):
            array_batch_1, resize_ratios = self._ImageDataset__load_image(
                self.__get_path_to_img(), target_shape, get_resize_ratios=True)
            self.__current_image_ratios.append(resize_ratios)
            input_array = np.concatenate((input_array, array_batch_1), axis=0)
            self.__current_image_ratios.append(self._ImageDataset__get_resize_ratios())
