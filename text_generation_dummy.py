...
class TextGenerationDummy:
    """
    A class providing facilities for preprocessing and postprocessing of text generation dummy dataset.
    """

    def __init__(self, model_name: str, tokenize_func, batch_size: int, text_generation_dataset_path: None):

        if text_generation_dataset_path is None:
            env_var = "TEXT_GENERATION_PATH"
            mrpc_dataset_path = utils.get_env_variable(
                env_var, f"Path to TEXT GENERATION DUMMY dataset directory has not been specified with {env_var} flag")
