    ...
    def run_single_pass(tf_runner, imagenet):
        ...
        for i in range(batch_size):
            imagenet.submit_predictions(
                i,
                imagenet.extract_top1(output["InceptionResnetV2/AuxLogits/Logits/BiasAdd:0"][i]),
                imagenet.extract_top5(output["InceptionResnetV2/AuxLogits/Logits/BiasAdd:0"][i])
            )

    dataset = ImageNet(batch_size, "RGB", images_path, labels_path,
                       pre_processing="Inception", is1001classes=True)
    runner = TFFrozenModelRunner(model_path, ["InceptionResnetV2/AuxLogits/Logits/BiasAdd:0"])
