    ...
    def run_single_pass(tf_runner, imagenet):
        ...
        for i in range(batch_size):
            imagenet.submit_predictions(
                i,
                imagenet.extract_top1(output["resnet_v2_50/predictions/Reshape_1:0"][i]),
                imagenet.extract_top5(output["resnet_v2_50/predictions/Reshape_1:0"][i])
            )

    dataset = ImageNet(batch_size, "RGB", images_path, labels_path,
                       pre_processing="Inception", is1001classes=True)
    runner = TFFrozenModelRunner(model_path, ["resnet_v2_50/predictions/Reshape_1:0"])
