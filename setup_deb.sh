...
if echo "$PYTHON3_VERSION" | grep -q "(python 3.8)"; then
   ...
   pip3 install --no-deps --upgrade https://ampereaimodelzoo.s3.eu-central-1.amazonaws.com/SimpleITK_aarch64-2.1.1-cp39-cp39-linux_aarch64.whl
elif echo "$PYTHON3_VERSION" | grep -q "(python 3.10)"; then
   pip3 install --no-deps --upgrade https://ampereaimodelzoo.s3.eu-central-1.amazonaws.com/SimpleITK_aarch64-2.1.1-cp310-cp310-linux_aarch64.whl
else
...
python3 $SCRIPT_DIR/setup_utils/install_frameworks.py
log "done.\n"
